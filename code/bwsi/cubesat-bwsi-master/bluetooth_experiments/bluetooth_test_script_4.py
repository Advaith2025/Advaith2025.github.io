...
def find_ground_station():
    """Improved method to find the ground station using proper service discovery"""
    logger.info("Searching for CubeSat Ground Station service...")
    
    # First try service discovery with retry
    for attempt in range(2):
        try:
            service_matches = bluetooth.find_service(name="CubeSatGroundStation", 
            ...
            if service_matches:
                ...
                logger.info(f"Service discovery attempt {attempt+1} failed. Waiting before retry.")
                time.sleep(2)  # Short delay between retries
        except Exception as e:
            logger.warning(f"Service discovery error on attempt {attempt+1}: {e}")
    
    # If service discovery fails, fall back to device discovery
    logger.info("Service discovery failed. Searching for nearby devices...")
    try:
        ...
        # Look for devices with recognizable names
        ...
        # If no obvious ground station, ask user which device to try
        print("\nAvailable Bluetooth devices:")
        for i, (addr, name) in enumerate(nearby_devices):
            print(f"{i+1}. {name} ({addr})")
        
        selection = input("\nSelect a device to connect to (number) or 'q' to quit: ").strip()
...
def create_and_connect_socket(addr, port, timeout=10.0):
    """Create and connect a Bluetooth socket with proper error handling"""
    sock = None
    try:
        # Create a new socket
        ...
        # Try to connect
        logger.info(f"Connecting to {addr} on port {port}...")
...
def safe_send(sock, data, retries=3):
    """Safely send data with error checking and retry"""
    for attempt in range(retries):
        try:
            ...
            if bytes_sent < len(data):
                ...
                # Send the remainder
                remaining = safe_send(sock, data[bytes_sent:], retries-1)
                return bytes_sent + remaining if remaining else bytes_sent
            return bytes_sent
        except bluetooth.btcommon.BluetoothError as e:
...
def safe_recv(sock, buffer_size, timeout=None, expected_response=None, max_attempts=5):
    """Safely receive data with improved error handling"""
...
def test_bluetooth_client():
    """Test Bluetooth client connection to ground station with improved reliability"""
    
    # Step 1: Find the ground station
    addr, port = find_ground_station()
    if not addr:
        logger.error("Could not find a ground station to connect to")
        return False
    
    # Step 2: Attempt to connect the socket
    socket, connected = create_and_connect_socket(addr, port)
    if not connected:
        # Try alternative port if first attempt fails
        logger.info("Trying alternative port (port 1)...")
        socket, connected = create_and_connect_socket(addr, 1)
        if not connected:
            logger.error("Failed to connect on both primary and alternative ports")
            return False
    ...
    try:
        ...
        bytes_sent = safe_send(socket, json_data)
        if not bytes_sent:
        ...
        # Step 4: Create test image - using a very small image for testing
        ...
        # Step 5: Send image start message
        timestamp = time.time()
        start_message = {
            "type": "image_start",
            "timestamp": timestamp,
        ...
        bytes_sent = safe_send(socket, json_data)
        if not bytes_sent:
            logger.error("Failed to send image start message")
            return False
        ...
        for i in range(0, image_size, chunk_size):
            ...
            if not bytes_sent:
                logger.error(f"Failed to send chunk at offset {i}")
                break
        ...
        # Step 7: Wait for final acknowledgment
        ...
        if response:
            ...
            if b"READY_FOR_CLASSIFICATION" in response:
                ...
                bytes_sent = safe_send(socket, json_data)
                if not bytes_sent:
                    logger.error("Failed to send classification data")
                    return False
        ...
        # Step 9: Always properly close the socket
